...
def load_latest_cluster_dataset():
    """Load latest cluster dataset from repository Data folder."""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    repo_root = os.path.abspath(os.path.join(current_dir, '..', '..'))
    data_dir = os.path.join(repo_root, 'Data')

    expense_path = os.path.join(data_dir, 'Financial_Transactions_Cluster_Expense.csv')
    income_path = os.path.join(data_dir, 'Financial_Transactions_Cluster_Income.csv')
    legacy_path = os.path.join(data_dir, 'Financial_Transactions_Cluster.csv')

    # Prefer latest split datasets (Expense + Income), fallback to legacy single file.
    if os.path.exists(expense_path) and os.path.exists(income_path):
        df_expense = pd.read_csv(expense_path)
        df_income = pd.read_csv(income_path)
        df = pd.concat([df_expense, df_income], ignore_index=True)
        source = f"{os.path.basename(expense_path)} + {os.path.basename(income_path)}"
    elif os.path.exists(legacy_path):
        df = pd.read_csv(legacy_path)
        source = os.path.basename(legacy_path)
    else:
        raise FileNotFoundError(
            "Dataset cluster tidak ditemukan di folder Data. "
            "Pastikan file Financial_Transactions_Cluster_Expense.csv dan "
            "Financial_Transactions_Cluster_Income.csv tersedia."
        )
...
def main():
    ...
    # 1. Memuat dataset terbaru dari folder Data
    try:
        df, dataset_source = load_latest_cluster_dataset()
        print(f"Dataset sumber: {dataset_source}")
    ...
    # Kolom label teks tidak digunakan sebagai fitur model.
    if 'Label' in df.columns:
        df = df.drop('Label', axis=1)

    if 'Cluster' not in df.columns:
        print("Error: kolom target 'Cluster' tidak ditemukan di dataset.")
        return
